...
from model.model import resnet56
...
def prune_model(seed: int = 42):
    ...
    output_cache_dir = os.path.join(base_dir, 'cache_resnet56_cifar10_pruned_0.78')
    os.makedirs(output_cache_dir, exist_ok=True)

    batch_size = 128
    num_workers = 16

    local_model_path = '/workspace/OTOv2_v2/test/pretrain_resnet56_cifar10/model/resnet56-4bfd9763.th'
    print(f"加载预训练 ResNet56 权重: {local_model_path}")
    model = resnet56()
    ...
    transform_train = transforms.Compose([
        ...
        # Cutout：用 RandomErasing 近似实现
        transforms.RandomErasing(
            p=0.5,               # 50% 概率做一次擦除
            scale=(0.02, 0.2),   # 擦除面积占比
            ratio=(0.3, 3.3),    # 高宽比
            value=0.0,
            inplace=False,
        ),
    ...
    nodes_to_keep_unpruned = ['node-333']
    ...
    print(f"剪枝前 FLOPs(thop): {flops_before_m:.2f} M")
...
def finetune_model(seed: int = 42):
    ...
    pruned_cache_dir = base_dir / "cache_resnet56_cifar10_pruned_0.78"
    ...
    finetuned_model_dir = os.path.join(base_dir, 'finetuned_resnet56_sparsity_0_78')
    os.makedirs(finetuned_model_dir, exist_ok=True)
    finetuned_model_path = os.path.join(finetuned_model_dir, 'resnet56_cifar10_finetuned.pt')
    ...
    baseline_model_path = '/workspace/OTOv2_v2/test/pretrain_resnet56_cifar10/model/resnet56-4bfd9763.th'
    baseline_model = resnet56()
