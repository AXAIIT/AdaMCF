...
from cifar100_dataloader import get_cifar100_train_loader, get_cifar100_test_loader
from model.resnet18 import load_pretrained_resnet18
...
def prune_model(seed: int = 42):
    ...
    output_cache_dir = base_dir / "cache_resnet18_cifar100_pruned_0.7"
    output_cache_dir.mkdir(parents=True, exist_ok=True)

    batch_size = 256
    num_workers = 16
    data_root_dir = "/workspace/data/cifar100"
    weights_path = "/workspace/OTOv2/test/pretrain_resnet18_cifar100/model/best_model.pth"

    model = load_pretrained_resnet18(weights_path=weights_path, device=device)
    model.to(device)
...
def finetune_model(seed: int = 42):
    ...
    pruned_cache_dir = base_dir / "cache_resnet18_cifar100_pruned_0.7"
    ...
    finetuned_model_dir = base_dir / "finetuned_resnet18_cifar100_sparsity_0_7"
    finetuned_model_dir.mkdir(parents=True, exist_ok=True)
    finetuned_model_path = finetuned_model_dir / "resnet18_cifar100_finetuned.pt"
    ...
    baseline_model_path = ("/workspace/OTOv2/test/pretrain_resnet18_cifar100/model/best_model.pth")
    baseline_model = load_pretrained_resnet18(weights_path=baseline_model_path, device=device)
