...
def finetune_model(seed: int = SEED):
    ...
    model_ema = ModelEmaV2(model, decay=0.99996, device=device)
    ...
    for epoch in range(FINETUNE_EPOCHS):
        ...
        if acc1_ema > best_top1:
            best_top1 = acc1_ema
            logger.info(f"[FT] 更新最佳 EMA Top-1: {best_top1:.2f}% -> 保存 {finetuned_model_path}")
            to_save = model_ema.module.module if isinstance(model_ema.module, torch.nn.DataParallel) else model_ema.module
